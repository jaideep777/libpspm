...
int main(){
	...
	ofstream foutx("abm_wave_x.txt");
	ofstream fouty("abm_wave_y.txt");

	ofstream fout("abm_wavemodel_equil.txt");
	...
	for (double t=0; t <= 1; t=t+0.1) {
		...
		vector<double> vx = S.getDensitySpecies1D(0, 0, breaks, Spline::CONSTANT);
		for (auto x : vx) foutx << x << "\t";
		foutx << endl;

		vector<double> vy = S.getDensitySpecies1D(0, 1, breaks, Spline::CONSTANT);
