...
int main(){
	...
	Solver<TestModel> S(5, 0,1, SOLVER_EBT);
	...
	//S.print();	
	// expected to be: 
	vector<double> exp = {0, .1, .3, .5, .7, .9, 0, 0.245885, 0.0980358, 0.0395062, 0.0143676, 0.00306934};
	if (!almostEqual(S.state, exp)) return 1;

	S.state[9] = S.state[10] = 1e-11;
	//S.print();

	S.removeDeadCohorts_EBT();
	//S.print();
	vector<double> exp2 = {0, .1, .3, .9, 0, 0.245885, 0.0980358, 0.00306934};
	if (!almostEqual(S.state, exp2)) return 1;
