...
plot_dists("outputs/abm_3000_2", "ABM")
# dev.off()
...
seeds_abm = read.delim("outputs/abm_3000_2/seed_rains.txt", header = F)

plot_seeds = function(y, title){
  matplot(y = y, x=seeds_fmu$V1, type="l", lty=1, lwd=2, col=scales::alpha(c("purple", "green3", "mediumspringgreen", "cyan3", "darkgoldenrod2"), alpha=0.7), xlab="Time (years)", ylab="Seed rain")
  mtext(title, line=1)
}
...
plot_seeds(cbind(seeds_fmu$V2, seeds_ifmu$V2, seeds_ifmu1000$V2, seeds_ebt$V2, seeds_abm$V2), "Species 1")
plot_seeds(cbind(seeds_fmu$V3, seeds_ifmu$V3, seeds_ifmu1000$V3, seeds_ebt$V3, seeds_abm$V3), "Species 2")
plot_seeds(cbind(seeds_fmu$V4, seeds_ifmu$V4, seeds_ifmu1000$V4, seeds_ebt$V4, seeds_abm$V4), "Species 3")
# dev.off()
