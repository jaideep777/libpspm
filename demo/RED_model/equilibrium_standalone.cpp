...
// Constants
const double phi_g = 0.7134;
const double g0 = 0.0838;
const double phi_a = 0.749;
const double a0 = 0.396;
const double E0 = 0.0001;
const double myu0 = 0.035;
const double alpha = 0.1;

// Custom log-spaced sequence
Elementwise<double> my_log_seq(double x0, double xf, int N) {
    Elementwise<double> grid;
    for (int i = 0; i < N; ++i) {
        grid.push_back(exp(log(x0) + (double(i) / (N - 1)) * (log(xf) - log(x0))));
    }
    return grid;
}

// Trapezoid integration
double trapz(const Elementwise<double>& y, const Elementwise<double>& x) {
    double sum = 0.0;
...
Elementwise<double> cumtrapz(const Elementwise<double>& y, const Elementwise<double>& x) {
    Elementwise<double> result(x.size(), 0.0);
    for (size_t i = 1; i < x.size(); ++i) {
...
double betax(double x, const Elementwise<double>& Ez) {
    return (alpha/(1-alpha)) * g(x) * (1 - Ez[0]);
}

double Ac(double x) {
    return a0 * pow(x, phi_a);
}

// Environment calculation function
...
std::pair<Elementwise<double>, Elementwise<double>> solve_u() {
    Elementwise<double> x = my_log_seq(1.0, 1e6, 50);
    Elementwise<double> z = my_log_seq(1.0, 1e6, 200);
