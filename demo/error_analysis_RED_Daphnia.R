...
# Error analysis
methods = c("FMU", "IFMU", "IFMU(O2)", "EBT", "CM", "ABM")
cols = col=c("purple", "green3", "mediumspringgreen", "darkgoldenrod2", "pink", "#2b8cbe") # Insert "pink",  in second last position for CM
...
err_abm %>% with(points(Eb~Nf, type="o", col=cols[5], lwd=2))
...
err_abm %>% with(points(Eb~tsys, type="o", col=cols[5], lwd=2))
legend(x = 1.5, y = 1e-4, legend = methods, col=cols, lty=1, lwd=2, bty = "n")
...
err_ifmu2 %>% with(points(Eb~Nf, type="o", col=cols[3], lwd=2))
err_ebt %>% with(points(Eb~Nf, type="o", col=cols[4], lwd=2))
err_cm %>% with(points(Eb~Nf, type="o", col=cols[5], lwd=2))
err_abm %>% with(points(Eb~Nf, type="o", col=cols[6], lwd=1))
...
err_ifmu2 %>% with(points(Eb~tsys, type="o", col=cols[3], lwd=2))
err_ebt %>% with(points(Eb~tsys, type="o", col=cols[4], lwd=2))
err_cm %>% with(points(Eb~tsys, type="o", col=cols[5], lwd=2))
err_abm %>% with(points(Eb~tsys, type="o", col=cols[6], lwd=1))
