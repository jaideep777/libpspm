...
A fixed grid (mesh) with $J$ cells is used to discretize the physiological variable $x$. The edges of the gridcells are at $x_j, j \in [0,J]$, and centres are at $X_j = (x_j+x_{j+1})/2$. The mesh is created during initialization. Note that $x_0 = x_b, x_J = x_m$. The FMU method solves for the solution $u(x)$ at the cell centres $X_j$.

Integrating the PDE over a grid cell $j$, we get

$$
\frac{\partial}{\partial t} \int_{x_{j}}^{x_{j+1}} u(x)dx = -[gu]_{x_{j}}^{x_{j+1}} -  \int_{x_{j}}^{x_{j+1}} \mu(x)u(x)dx
$$

Lets us define the average value of $u(x)$ over the cell $j$ as $U_j$. Therefore, 
$$
h_j U_j = \int_{x_{j}}^{x_{j+1}} u(x)dx
$$
where $h_j$ is the width of the cell, $h_j=x_{j+1}-x{j}$.

We also approximate the integrals on the RHS and the boundary counditions using midpoint quadrature rule:
$$
\int_{x_{j}}^{x_{j+1}} \mu(x)u(x)dx = h_j \mu(X_j) U_j \\
\int_{x_{b}}^{x_{m}} \beta(x)u(x)dx = \int_{x_{0}}^{x_{J}} \beta(x)u(x)dx = \sum_{j=0}^{J-1} h_j \beta(X_j) U_j
$$

Therefore, we can write,
$$
\frac{\partial}{\partial t} U_j = -\frac{[gu]_{x_{j}}^{x_{j+1}}}{h_j} - \mu(X_j) U_j
$$
...
$$
u_3 = u^{o1}_3 + \phi (u^{o2}_3-u^{o1}_3)
$$
...
$$
r = \Delta U_{downwind} / \Delta U_{upwind}
$$

where, e.g., 
$$
\Delta U_{3,downwind} = \frac{U_3-U_2}{X_3-X_2}
$$
and

$$
\Delta U_{3,upwind} = \frac{U_2-U_1}{X_2-X_1}
$$
...
r = \Delta U_{downwind} / \Delta U_{upwind} \\ 
\Delta U_{downwind} = \frac{U_i-U_{i-1}}{X_{i}-X_{i-1}} \\
\Delta U_{upwind} = \frac{U_{i-1}-U_{i-2}}{X_{i-1}-X_{i-2}} \\
$$
...
r = \Delta U_{downwind} / \Delta U_{upwind} \\ 
\Delta U_{downwind} = \frac{U_i-U_{i-1}}{x_{i+1}-x_{i-1}} \\
\Delta U_{upwind} = \frac{U_{i-1}-U_{i-2}}{x_{i}-x_{i-2}} \\
$$
...
$$
u_0 = \frac{s_e}{g(x_0)} \int_{x_{0}}^{x_{J}} \beta(x)u(x)dx = \frac{s_e}{g(x_0)} \sum_{j=0}^{J-1} h_j \beta(X_j) U_j
$$